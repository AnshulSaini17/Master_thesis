...
from tensordyne.nn.modules.normalization import BufferizedRMSNorm, _build_rms_norm
...
class TensordyneMultiHeadSelfAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.0, use_rmsnorm=True, eps=1e-8, causal=False):
        ...
        # Pre-norm (optional)
        if use_rmsnorm:
            self.norm = BufferizedRMSNorm(embed_dim, eps=eps, upcast_input_to_fp32=True)
    ...
    def build(self, x, attn_mask=None):
        ...
        # Reshape to heads: (batch, seq, embed_dim) -> (batch, num_heads, seq, head_dim)
